/*
  # Fix Award Carrier Function to Copy CSP Priority

  1. Problem
    - When awarding a carrier from a Priority 1 (or other priority) CSP event
    - The created tariff doesn't preserve the priority
    - It defaults to "Customer Direct" instead of "P1 CSP" or other priority
    
  2. Solution
    - Update award_carrier_with_tariff to read the CSP event's priority
    - Copy the priority value to the tariff's rocket_csp_subtype field
    - This ensures P1 CSP, P2 CSP, etc. are properly reflected in the tariff
...
DECLARE
  ...
  v_expiry_date date;
BEGIN
  -- Get assignment details including CSP priority
  SELECT
    cec.carrier_id,
    c.name,
    cec.csp_event_id,
  ...
  v_family_name := COALESCE(v_customer_name, 'Unknown Customer') || ' - ' || COALESCE(v_carrier_name, 'Unknown Carrier');
  ...
  -- Set default dates (can be updated later in Tariff Detail page)
  v_effective_date := current_date;
  v_expiry_date := current_date + interval '1 year';

  -- Generate tariff reference ID
  ...
  -- Create proposed tariff (now including rocket_csp_subtype from CSP priority)
  INSERT INTO tariffs (
    tariff_reference_id,
    tariff_family_id,
    customer_id,
    ...
    'proposed',
    v_ownership_type,
    v_priority,  -- Copy CSP priority to rocket_csp_subtype
    v_effective_date,
    v_expiry_date,
    v_csp_event_id,
    v_mode,
...
GRANT EXECUTE ON FUNCTION award_carrier_with_tariff(uuid, uuid) TO authenticated;