/*
  # Create Award Carrier Function

  1. Overview
    - Creates a complete award flow function that handles carrier award and tariff creation
    - Automatically creates tariff family if needed
    - Creates proposed tariff and links it to the assignment
    - Logs activities to all relevant entities

  2. Function: award_carrier_with_tariff
    - Parameters:
      - p_assignment_id: The csp_event_carriers ID
      - p_awarded_by: The user awarding the carrier
    - Returns: JSON with tariff_id and tariff_reference_id
    - Atomic transaction: all-or-nothing

  3. Process Flow
    - Update csp_event_carriers status to 'awarded'
    - Resolve/create tariff_families record
    - Create proposed tariff with generated reference ID
    - Link tariff back to assignment
    - Log activities to CSP event, customer, carrier, and tariff
...
DECLARE
  ...
  v_csp_effective_date date;
  v_csp_expiry_date date;
...
BEGIN
  ...
  v_tariff_reference_id := generate_tariff_reference_id(
    v_customer_id,
    ARRAY[v_carrier_id],
    COALESCE(v_csp_effective_date, current_date)
  );

  -- Create proposed tariff
  INSERT INTO tariffs (
    reference_id,
    family_id,
    customer_id,
